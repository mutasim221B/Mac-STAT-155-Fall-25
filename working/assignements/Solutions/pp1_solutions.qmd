...
Discussions will vary. In general, students should point out that, overall, slightly over half of films pass the Bechdel test (are "ok" or "dubious"). Further, there should be some discussion of the films that don't pass. For example, among the films that fail the test, most are because two women characters don't talk to one another.
...
c. For every extra 1 million dollars spent on making the movie, the expected gross increases by roughly \$3.06 million.

d. It doesn't make sense to interpret the intercept in this model. This would be the expected gross of a movie that had a $0 budget. No such movies exist in this data set.
