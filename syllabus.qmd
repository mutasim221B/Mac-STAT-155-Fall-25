...
<style>
/* kill zebra striping on the schedule table only */
#schedule-table,
#schedule-table th,
#schedule-table td {
...
# No kableExtra needed; we'll emit a plain pipe table
library(dplyr)

# helper: HTML line breaks that also work in PDF via pandoc
lb <- function(...) paste(c(...), collapse = "<br>")
...
df <- tibble::tribble(
  ...
  "Normal distribution", "10–14",
  "", "", "Sampling distributions & the CLT", "",
  "", "", "Confidence intervals", "",
  "", "", "Hypothesis tests", ""
)

# Emit a plain Markdown (pipe) table:
knitr::kable(
  df,
  format = "html",                     # use HTML so the id attaches cleanly
  align  = c("l","l","l","c"),
  escape = FALSE,
  col.names = c("UNIT","GOAL","TOPIC","~WEEKS"),
  table.attr = "id='schedule-table'"
)
```
