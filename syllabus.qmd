...
rows_blank_weeks <- which(df$`~WEEKS` == "")

gt_tbl <- df |>
  gt() |>
  cols_align(align = "left") |>
  # 2) remove zebra striping
  opt_row_striping(row_striping = FALSE) |>
  tab_options(row.striping.background_color = "transparent") |>
  # render <br> and R^2
  fmt_markdown(columns = c(GOAL, TOPIC)) |>
  # draw standard table borders/row rules
  opt_table_lines() |>
  # 3) hide horizontal rules in ~WEEKS when the cell is empty
  tab_style(
    style = cell_borders(sides = c("top","bottom"), color = "transparent"),
    locations = cells_body(columns = `~WEEKS`, rows = rows_blank_weeks)
  )
