...
<!-- Nothing clear; With the small number of unique values of the predictor variable, all of the points are bunched up on each other. -->
...
<!-- *Highest median: premium cut; *Interestingly, although “Ideal” suggests top quality- median price is actually lower than Premium and closer to Good/Very Good; *Premium and Very Good cuts show wider spreads, meaning more variation in prices; *Many outliers -->
...
<!-- *Distribution is right-skewed; mean > median; *Highest peak in lower prices, another small bump in around 4k-5k; As price increases, density drops quickly -->
...
<!-- *All right skewed; *Ideal Cut- highest peak at very low prices (under $1k) - many Ideal-cut diamonds are sold in lower price range; *Fair cut (red): Distribution is flatter, shifted toward somewhat higher prices compared to Ideal; Ideal cut may not “most expensive”- matches boxplot: lower median price -->
...
<!-- What problem do you notice? Can we now interprate the relationship between price and cut as we could in boxplot? Why not? Hint: Area under curve -->
...
<!-- *Surprisingly, diamonds with the best cut (Ideal) tend to have lower average prices, while those with the worst cut (Fair) are often more expensive -> suggests might be another factor— size maybe incfleuncing price? -->
...
<!-- Group means approx. reflect what we see in the plots (easiest to see in the boxplots) -->
...
<!-- * Intercept is the expected value (mean) of the price for all diamonds with a cut quality that isn’t Fair (Good, Very Good, Premium, or Ideal, i.e. when cutFair = 0)–the same as we saw in exercise 3e; *When we add the intercept and coefficient for cutFair, we get 3919.69 + 439.06 = 4358.75–this is the mean price for all diamonds with a Fair cut quality that we saw in exercise 3e! Therefore, the coefficient of cutFair (439.06) is interpreted as the difference between the mean value of diamonds with a Fair cut quality and the mean value of diamonds with a higher cut quality. -->




## Exercise 5: Modeling trend using a categorical predictor with >2 categories
<!-- If have time, in group; o/w, next class -->
...
<!-- each category assigned to a separate indicator variable–cutGood = 1 when cut == "Good" and 0 otherwise, cutVery Good = 1 when `cut == “Very Good” and 0 otherwise, and so on. -->
