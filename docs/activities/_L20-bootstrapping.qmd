...
One goal was to explore the relationship of a fish's mercury concentration (Concen) with its size, specifically its Length:
...
In this spirit, statistical bootstrapping **doesn't make any probability model assumptions**.
It uses only the information from our one sample to approximate standard errors.
...
We'll obtain a **bootstrapping distribution** of $\hat{\beta}_1$ by taking many (500) different samples of 171 fish and exploring the degree to which $\hat{\beta}_1$ varies from sample to sample.
Let's try doing this as we did in past activities:
...
In practice, we take *one sample* of size n from the population.
To obtain a *bootstrapping distribution* of some sample estimate $\hat{\beta}$, we...
...
a. Use the bootstrap estimates of the `Length` slope coefficient to *approximate* the standard error of 0.05813, our original sample estimate. HINT: standard deviation    
...
In the past few activities, we've been exploring sampling variability and error.
These tools are critical in using our sample to make *inferences* about the broader population.
We'll explore inference more formally in the weeks ahead.
...
b. We can do better than visual approximations. Use the `fish_models_bootstrap` results to provide a more specific interval of plausible values for the actual `Length` coefficient. THINK: Do you think we should use the full range of observed bootstrap estimates? Just a fraction?
...
Some researchers claim that mercury content is associated with the length of a fish.
Let's use our bootstrapping results to test this hypothesis.
